use super::buddy_alloc::BuddyAllocatorManager;
...
fn untrusted_mem_allocate(size: usize) -> Result<(), String> {
    ...
    .unwrap();
    info!(
        "buddy_system_allocator: allocate [ 0x{:x} ] untrusted memory.",
        &chunk
    );
    let layout = Layout::from_size_align(chunk, 1)
        .unwrap()
        .align_to(std::mem::size_of::<*const std::ffi::c_void>())
        .unwrap();
    let mut mem_ptr: *mut std::ffi::c_void = ptr::null_mut();
    let sgx_status = unsafe {
        occlum_ocall_posix_memalign(&mut mem_ptr as *mut _, layout.align(), layout.size())
    };
    assert!(sgx_status == sgx_status_t::SGX_SUCCESS);
    assert!(mem_ptr != std::ptr::null_mut());
    unsafe {
...
fn untrusted_mem_free() {
    for range in MANAGER.fetch_memory_ranges().unwrap() {
        ...
        let sgx_status = unsafe { occlum_ocall_free(range as *mut std::ffi::c_void) };
        assert!(sgx_status == sgx_status_t::SGX_SUCCESS);
    }
}

fn alloc(size: usize) -> Result<*mut u8, String> {
    let ptr = if size > 0 {
        ...
        match ptr {
            Ok(ptr) => ptr.as_ptr(),
            Err(_) => {
                untrusted_mem_allocate(size.clone())?;
                unsafe { MANAGER.alloc(layout).unwrap().as_ptr() }
            }
        }
    } else {
        PTR_NULL as *mut u8
...
fn free(ptr: *mut u8, size: usize) -> Result<(), String> {
    if ptr as u64 != PTR_NULL {
        let layout = Layout::from_size_align(size, 1).unwrap();
        unsafe {
            MANAGER.dealloc(NonNull::new(ptr).unwrap(), layout);
        }
    }
    Ok(())
}
...
fn exec<T>(fd: i32, cmd: &u32, arg: *const u8) -> Result<i32, String> {
    ...
    free(ptr_u as *mut u8, mem::size_of::<T>())?;
            ...
            *dst = alloc(size)? as u64;
            ...
            if base_src.name == 0 {
                ...
                if param_src.param.size > 0 {
                    ...
                    param_dst.param.value = PTR_NULL;
                }
            }
            Ok(())
        });
...
fn i915_gem_userptr_ioctl(fd: i32, cmd: &u32, arg: *const u8) -> Result<i32, String> {
    ...
    let arg_t = unsafe { &mut *(ptr_t as *mut drm_i915_gem_userptr) };
...
fn drm_default_ioctl(fd: i32, cmd: &u32, arg: *const u8) -> Result<i32, String> {
...
pub fn ioctl(fd: i32, cmd: &u32, arg: *const u8) -> Result<i32, String> {
    match cmd.to_owned() {
...
extern "C" {
    ...
    fn occlum_ocall_free(ptr: *mut std::ffi::c_void) -> sgx_status_t;
}
